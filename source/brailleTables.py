...
def initialize():
	...
	for dir_ in reversed(customDirs):
		# Redundant check, in case an addon added an external directory to
		# the list.
...
def _getCustomTablesDirs():
	...
	def candidates():
		...
		yield os.path.join(globalVars.appArgs.configPath, "brailleTables")
		import addonHandler
...
def _loadCustomTablesConfig(data):
	"""Load the configuration data for custom tables.
	
	The expected data is a dictionary, typically loaded from a JSON file,
	of the form:
	{
		"BrailleTablesConfig": {
	...
	All keys are optional and default to the default of L{addTable}.
	...
	"""
	for fileName, tableCfg in data["BrailleTablesConfig"].items():
		addTable(
			fileName,
			displayName=_getCustomBrailleTableDisplayName(fileName, tableCfg),
...
def _getCustomBrailleTableDisplayName(fileName, tableCfg):
	...
	if not isinstance(value, dict):
		raise ValueError(
			"Unexpected displayName value for custom braille table "
			"{fileName}: {value}".format(**locals())
		)
